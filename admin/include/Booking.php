...
class Booking extends DB_Object
{
    ...
    public static function getGroom($booking_id) {
        global $db;
        $sql = "SELECT groom FROM tblweddingbook WHERE booking_id = $booking_id";
        $result_set = $db->query($sql);
    ...
    public static function count_booking() {
        global $db;
        $sql = "SELECT count(booking_id) FROM tblweddingbook inner join tblaccounts_detail ON tblweddingbook.user_id = tblaccounts_detail.user_id WHERE tblaccounts_detail.status = 'confirm' ORDER BY booking_id ASC";
